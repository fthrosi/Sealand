...
interface PortfolioFormEditProps {
  onSuccess: () => void;
  portfolio: PortfolioOutput;
}
export default function PortfolioFormEdit({
  onSuccess,
  portfolio,
}: PortfolioFormEditProps) {
  const close = useUIStore((state) => state.close);
  const [imagePreview, setImagePreview] = useState<string | null>(
    `${process.env.NEXT_PUBLIC_API_URL}/${portfolio.img_url}`,
  );
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: PortfolioEditType) => {
  ...
  return (
    <Modal title="Form Edit Portfolio">
            ...
            {imagePreview ? (
              <div className="relative">
                ...
                <button
                  type="button"
                  onClick={() => {
                    setImagePreview(`${process.env.NEXT_PUBLIC_API_URL}/${portfolio.img_url}`);
                    setValue("image", undefined as any);
                  }}
                  className={`${imagePreview === `${process.env.NEXT_PUBLIC_API_URL}/${portfolio.img_url}` ? "hidden" : ""} absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full hover:bg-red-600 transition-colors`}
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
