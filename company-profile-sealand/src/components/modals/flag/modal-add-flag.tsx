...
export default function FlagAdd({ onClose, onSuccess }: FlagFormProps) {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: FlagType) => {
    try {
      const response = await addFlag(data);
      if (response.data.success) {
        toast.success("Bendera berhasil ditambahkan");
        reset();
        onSuccess();
        onClose();
      } else {
        toast.error("Gagal menambahkan bendera");
      }
      console.log(data);
    } catch (error) {
