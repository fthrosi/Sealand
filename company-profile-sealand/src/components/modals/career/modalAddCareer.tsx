...
import { addCareer } from "@/api/career";
...
export default function CareerAdd({ onClose, onSuccess }: CareerFormProps) {
  ...
  const onSubmit = async (data: CareerType) => {
    try {
      const response = await addCareer(data);
