...
import { GalleryEditSchema, type GalleryEditType} from "@/schema/gallerySchema";
...
export default function GalleryFormEdit({ onSuccess, gallery }: GalleryFormEditProps) {
  const option = [
    {
        value: "Landscape",
        label: "Landscape"
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(
    `${process.env.NEXT_PUBLIC_API_URL}/${gallery.img_url}`,
  );
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: GalleryEditType) => {
    ...
    try {
      const response = await updateGallery(gallery.id, formData);
      toast.success("Gallery berhasil diperbarui");
      onSuccess();
      close();
    } catch (error) {
      toast.error("Terjadi kesalahan saat memperbarui gallery");
    }
  };
  return (
    <Modal title="Form Edit Gallery">
            ...
            {imagePreview ? (
              <div className="relative">
                ...
                <button
                  ...
                  className={`${imagePreview === `${process.env.NEXT_PUBLIC_API_URL}/${gallery.img_url}` ? "hidden" : ""} absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full hover:bg-red-600 transition-colors`}
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
